// opencode-site 从 SQLite 生成单页 HTML 站点 (docs/index.html)
package main
...
func main() {
	...
	fmt.Printf("已生成 %s (%s 条)\n", outPath, thousands(total))
}
...
tr.subtotal{font-weight:700;background:#f9fafb}
...
<p>生成时间 {{.GeneratedAt}} · 数据范围 {{.Range}} · 共 {{.Total}} 条 · 输入tokens = input + cache_read + cache_write</p>
</header>
<nav>
<button class="active" onclick="openTab('month')">按月</button>
<button onclick="openTab('model')">按模型</button>
<button onclick="openTab('monthModel')">按月×模型</button>
<button onclick="openTab('cache')">缓存</button>
<button onclick="openTab('daily')">每日 ({{.DailyMonth}})</button>
<button onclick="openTab('peak')">DeepSeek 峰谷 ({{.PeakStart}}起)</button>
</nav>
<main>
<!-- 按月 -->
...
<!-- 按月x模型 -->
<div id="tab-monthModel" class="tab"><div class="card"><h2>按月 × 模型 <span class="badge">含每月小计 · 每亿输入tok</span></h2>
...
<p class="note">小计已在 Markdown 导出中保留，页面按月连续展示可配合排序查看。</p>
</div></div>
...
<script>
function openTab(n){document.querySelectorAll('.tab').forEach(e=>e.classList.remove('active'));document.getElementById('tab-'+n).classList.add('active');document.querySelectorAll('nav button').forEach(b=>b.classList.remove('active'));event.currentTarget.classList.add('active')}
